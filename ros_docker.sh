...
function usage() {
    print_info "Usage: run_dev.sh" {ros_dev directory path OPTIONAL}
    print_info "Copyright (c) 2021-2022, NVIDIA CORPORATION."
}

ROS_DEV_DIR="$1"
if [[ -z "$ROS_DEV_DIR" ]]; then
    ROS_DEV_DIR="$HOME/workspaces/ros2_ws"
    if [[ ! -d "$ROS_DEV_DIR" ]]; then
        ROS_DEV_DIR=$(pwd)
    fi
    print_warning "ros_dev not specified, assuming $ROS_DEV_DIR"
else
    shift 1
fi
...
if [ "$(docker ps -a --quiet --filter status=running --filter name=$CONTAINER_NAME)" ]; then
    ...
    docker exec -i -t -u admin --workdir /workspaces/ros2_ws $CONTAINER_NAME /bin/bash $@
    exit 0
fi
...
docker run -it --rm \
    ...
    --network host \
    ${DOCKER_ARGS[@]} \
    -v $ROS_DEV_DIR:/workspaces/ros2_ws/ \
    -v /dev/*:/dev/* \
